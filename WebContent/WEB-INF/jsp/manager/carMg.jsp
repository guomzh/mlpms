		...
		<div class="container" style="width: 1500px">
			...
			<form id="searchForm">
				<div class="input-group" style="float:right;width:200px;">
					  ...
					  <input type="text" name="carId" class="form-control" placeholder="关键字车牌号">
			...
			<div class="panel panel-info" style="margin-top: 65px;">
				<div class="panel-heading">
					<h3 class="panel-title"><i class="glyphicon glyphicon-volume-down"></i> 车辆管理</h3>
				</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form:form  modelAttribute="car" id="editForm">
          <div class="form-group">
            <label for="recipient-name" class="control-label">车牌号:</label>
            <form:input type="text" class="form-control" id="carNum" name="carNum" path="carNum"/>
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">车辆颜色:</label>
            <form:input type="text" class="form-control" id="color" name="color" path="color"/>
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">车辆品牌:</label>
            <form:input type="text" class="form-control" id="brand" name="brand" path="brand"/>
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">车主ID:</label>
            <form:input type="text" class="form-control" id="userId" name="userId" path="userId"/>
          </div>
           <div class="form-group">
            <label for="message-text" class="control-label">车辆描述:</label>
            <form:input type="text" class="form-control" id="description" name="description" path="description"/>
          </div>
...
$(function(){
	...
	datagrid.on("click","button",function(){
		var carNum = $(this).parent().prev().prev().prev().prev().prev().html();
		if($(this).attr("id") == "delete"){
		$.ajax("${pageContext.request.contextPath}/manager/deleteCar",// 发送请求的URL字符串。
				{
		...
		alert("删除用户"+carNum+"成功！");
		
	}
		else{
			$.post("${pageContext.request.contextPath}/manager/loadEditCar?carNum="+carNum,null,
					function(data){
				$("#editType").val(1);

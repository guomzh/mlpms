		...
		<div class="container" style="width: 1500px">
			...
			<form id="searchForm">
				<div class="input-group" style="float:right;width:200px;">
			          ...
			          <input type="text" name="key"  class="form-control" placeholder="关键字户型描述">
			...
			<div class="panel panel-info" style="margin-top: 65px;">
				<div class="panel-heading">
					<h3 class="panel-title"><i class="glyphicon glyphicon-volume-down"></i> 户型管理</h3>
				</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">户型信息</h4>
      </div>
      <div class="modal-body">
        <form:form  modelAttribute="houseType" id="editForm">
        <div class="form-group">
            <label for="recipient-name" class="control-label">户型编号:</label>
          ...
          <div class="form-group">
            ...
            <form:input type="text" class="form-control" id="area" name="area" path="area"/>
          </div>
          <div class="form-group">
            <label for="message-text" class="control-label">户型描述:</label>
            <form:textarea style="height:300px" class="form-control" id="description" name="description" path="description"/>
          </div>
...
$(function(){
	...
	$("#edit").click(function(){
		$('#editForm')[0].reset();
		$("#houseType").removeAttr("readonly");
		$("#editType").val(0);
	});
	
	datagrid.on("click","button",function(){
		var houseType = $(this).parent().prev().prev().prev().html();
		if($(this).attr("id") == "delete"){
		$.ajax("${pageContext.request.contextPath}/manager/deleteHouseType",// 发送请求的URL字符串。
				{
				dataType : "json", // 预期服务器返回的数据类型。
	   			type : "post", //  请求方式 POST或GET
			   contentType:"application/json", //  发送信息至服务器时的内容编码类型
			   // 发送到服务器的数据。
			   data:JSON.stringify({houseType:houseType}),
			   async:  true , // 默认设置下，所有请求均为异步请求。如果设置为false，则发送同步请求
		});

		alert("删除户型"+houseType+"成功！");
		searchForm.trigger("submit");
		
	}
		else{
			$.post("${pageContext.request.contextPath}/manager/loadEditHouseType?houseType="+houseType,null,
					function(data){
					$("#editType").val(1);
					$("#houseType").val(data.houseType);
					$("#houseType").attr("readonly", "readonly");
					$("#area").val(data.area);

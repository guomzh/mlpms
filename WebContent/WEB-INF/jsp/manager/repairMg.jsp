	...
	<%@ include file="../common/navMg.jsp"%>
		<div class="container" style="width: 1500px">
						...
						<div class="col-md-3 column">
							<div class="btn-group">
								 <button class="btn btn-default" style="width: 80px; ">报修状态</button> 
			...
			<div class="panel panel-info" style="margin-top: 65px;">
				<div class="panel-heading">
					<h3 class="panel-title"><i class="glyphicon glyphicon-volume-down"></i> 报修管理</h3>
				</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="exampleModalLabel">报修信息</h4>
      </div>
      <div class="modal-body">
        <form:form  modelAttribute="repair" id="editForm">
         <div class="form-group">
            <label for="recipient-name" class="control-label">报修编号:</label>
            <form:input type="text" class="form-control" id="repairId" name="repairId" path="repairId"/>
          </div>
       <div class="form-group">
            <label for="message-text" class="control-label">报修状态:</label>
	            <form:select  class="form-control" style="width:143px;" path="typeId">    			
	    			<form:option value="0">审核中</form:option>
	    			<form:option value="1">处理中</form:option>	
	    			<form:option value="2">已处理</form:option>						    			
				</form:select>
          </div>
        <div class="form-group">
            <label for="recipient-name" class="control-label">报修内容:</label>
            <form:textarea readonly="readonly" style="height:200px" class="form-control" id="repairContent" name="repairContent" path="repairContent"/>
          </div>
          <div class="form-group">
            <label for="recipient-name" class="control-label">反馈内容:</label>
            <form:textarea style="height:300px" class="form-control" id="repairResponse" name="repairResponse" path="repairResponse"/>
          </div>
...
$(function(){
	...
	$("#done").click(
	 ...
	 datagrid.on("click","button",function(){
			var repairId = $(this).parent().prev().prev().prev().prev().prev().prev().prev().html();
			if($(this).attr("id") == "delete"){
			$.ajax("${pageContext.request.contextPath}/manager/deleteRepair",// 发送请求的URL字符串。
					{
			...
			alert("删除报修"+repairId+"成功！");
			
		}
			else{
				$.post("${pageContext.request.contextPath}/manager/loadEditRepair?repairId="+repairId,null,
						function(data){
						var type = data.repairState;
						$("#typeId").val(data.repairState);
						$("#repairContent").val(data.repairContent);
						$("#repairResponse").val(data.repairResponse);
						$("#repairContent").attr("readonly", "readonly");
						$("#repairId").val(data.repairId);
						$("#repairId").attr("readonly", "readonly");
				},"json");
				$("#edit").click();
